...
Table \ref{rawdata} shows the raw values (in seconds) as been gathered during the benchmarks. Every value equals to a single run. The first two columns show the existing malware detection systems Anubis and Cuckoo. The other columns show the data of the proof of concept developped for this project. The URLs used for this benchmark are random selected from the top 100 of a worldwide list\footnote{http://www.alexa.com/topsites} with the most populair websites.
